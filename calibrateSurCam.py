...
class StreamingHandler(httpServer.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/':
            ...
            page += '</head>\n'
            page += '<body>\n'
            page += '<h1>Video from surCam Calibration</h1>\n'
            ...
            page += '  console.log("changing changeMast to", changeMask);\n'
            page += '  if (!changeMask) {\n'
            ...
            page += '  }\n'
            page += '}, false);\n'
            #page += "canvas.addEventListener('mouseup', function(event) {\n"
            #page += '}, false);\n'
    ...
    def do_POST(self):
        ...
        if self.path == '/index.html':
            ...
            #newMask = np.asarray(line.split("=")[1]).reshape(30, 41)
